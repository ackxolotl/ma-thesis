...
\Cref{chap:implementations} for our performance analysis, i.e. \texttt{ixy.rs},
...
PCIe connection). \Cref{tab:servers} lists the devices under test.
...
also modify the packet generator \Cref{fig:page-size-generator} to generate
...
different IOVA address widths. \Cref{fig:iova-address-widths-throughput} shows
the results of our measurements. We run the forwarder on two ports of a single
\ac{nic} (\Cref{fig:iova-address-widths-single-device-throughput}) and on two
distinct \acp{nic} (\Cref{fig:iova-address-widths-two-devices-throughput}).
...
(0.15~Mpps faster compared to \Cref{fig:baseline-perf}).
